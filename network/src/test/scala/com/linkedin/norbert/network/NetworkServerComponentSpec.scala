...
class NetworkServerComponentSpec extends SpecificationWithJUnit with Mockito with NetworkServerComponent
        ...
        with NetworkClientFactoryComponent with ChannelPoolComponent with RequestHandlerComponent {
  ...
  "NetworkServerComponent" should {
    "when instantiated with a node id" in {
      "connect to the cluster and listen to cluster events" in {
        ...
        val server = new NetworkServer(1)
        server.bind
    ...
    "when instantiated with an InetSocketAddress" in {
      "connect to the cluster and listen to cluster events" in {
        ...
        val server = new NetworkServer(isa)
        server.bind
