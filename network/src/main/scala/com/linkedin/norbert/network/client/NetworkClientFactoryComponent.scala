...
import com.linkedin.norbert.network.common.{NorbertResponseIterator, NorbertFuture, ClusterIoClientComponent}

/**
 * A component which provides a network client for interacting with nodes in a cluster.
 */
trait NetworkClientFactoryComponent extends ClusterIoClientComponent {
  this: ClusterComponent with LoadBalancerFactoryComponent =>
  ...
  class NetworkClientFactory private[network] (clusterIoClient: ClusterIoClient) extends Logging {
    ...
    def start: Unit = {
      ...
      if (startedSwitch.compareAndSet(false, true)) {
        ...
        cluster.start
        updateCurrentState(cluster.nodes)
