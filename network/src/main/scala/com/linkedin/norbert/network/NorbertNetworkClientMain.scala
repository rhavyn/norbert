  ...
  private class Main(clusterName: String, zooKeeperUrls: String) {
    ...
    private def processCommand(line: String) {
      ...
      command match {
        ...
        case "join" =>
          if (args.length < 4) {
            println("Error: Invalid syntax: join nodeId hostname port partition1 partition2...")
          } else {
            val nodeId :: address :: port :: partitions = args
            cluster.addNode(nodeId.toInt, new InetSocketAddress(address, port.toInt), partitions.map(_.toInt).toArray)
